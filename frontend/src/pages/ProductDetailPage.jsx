...
function ProductDetailPage() {
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      ...
      try {
        ...
        // 2. GANTI thumbnailUrl JADI thumbnail_url
        if (data.thumbnail_url) {
          setSelectedImage(`${backendUrl}${data.thumbnail_url}`);
        }
      } catch (error) {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const response = await fetch(`${backendUrl}/api/cart`, { // Endpointnya /api/cart (method POST)
        method: 'POST',
        headers: {
      ...
      // Opsional: Redirect ke halaman keranjang
      // navigate('/cart'); 
  ...
  // 3. LOGIC GAMBAR (Hanya Thumbnail dulu karena belum ada Multi-Image di backend)
  const galleryImages = [
    product.thumbnail_url,
    // ...(Array.isArray(product.imageUrls) ? product.imageUrls : []), // Backend belum support array gambar
  ].filter(Boolean);
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-5">
              ...
              {/* Thumbnail Kecil (Hidden dulu kalau cuma 1 gambar) */}
        ...
        <div className="bg-white p-6 rounded-lg shadow-md mt-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              />
              <div className="ml-4">
                <p className="font-bold text-gray-900">{product.seller?.username || 'Penjual'}</p>
                <p className="text-xs text-gray-500">Aktif Baru Saja</p>
              </div>
            </div>
            <div className="flex items-center space-x-4">
